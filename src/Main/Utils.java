...
public class Utils {
	...
	public static class ApplyButtonListener implements ActionListener {
		...
		private void performCalculation(String xHoursStr, String xMinutesStr, String xSecondsStr, String yHoursStr,
				String yMinutesStr, String ySecondsStr) {
			try {
				...
				if (zHours >= 0 && zHours < Program.time1.length && zMinutes >= 0 && zMinutes < Program.time1[0].length
						&& zSeconds >= 0 && zSeconds < Program.time1[0][0].length) {
					...
					if (x1 < Program.time1.length && y1 < Program.time1[0].length && z1 < Program.time1[0][0].length
							&& x2 < Program.time2.length && y2 < Program.time2[0].length
							&& z2 < Program.time2[0][0].length && x3 < Program.time3.length
							&& y3 < Program.time3[0].length && z3 < Program.time3[0][0].length) {
						...
						System.out.println("Data changed: time1[" + x1 + "][" + y1 + "][" + z1 + "] and time2[" + x2
								+ "][" + y2 + "][" + z2 + "]" + " and time3[" + x3 + "][" + y3 + "][" + z3 + "]");
					}
				}
	...
	public static void roll(int scrollAmount, int x, int y) throws AWTException {
		Robot robot = new Robot();
		robot.mouseMove(x, y);
		robot.delay(Program.delay_duration[0][1]);
